...
if [ -n "$(git status --porcelain)" ]; then
  cat <<EOF | git commit -q -F - -- pkgs/development/haskell-modules/hackage-packages.nix
...
This update was generated by hackage2nix $cabal2nix using the following inputs:
  ...
  - Stackage Nightly: https://github.com/fpco/stackage-nightly/commit/$stackagenightly
EOF
  git push -q
fi
