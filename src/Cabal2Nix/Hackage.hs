...
hackagePath (PackageIdentifier (PackageName name) version') ext = concat
  [ "http://hackage.haskell.org/packages/archive/"
  , name,  "/", version, "/", name
  , if ext == TarGz then '-' : version else ""
  , showExt ext
  ] where version = showVersion version'
...
readCabalFile path
  | "cabal://" `isPrefixOf` path = do
    let pid p = fromJust $ simpleParse p
